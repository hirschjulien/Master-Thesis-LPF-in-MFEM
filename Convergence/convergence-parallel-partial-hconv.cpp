...
int main(int argc, char *argv[])
{
    ...
    for (int it = 0; it < 10; it++) // *** CHANGED: unified loop counter
    {
        // *** CHANGED: choose refinement/order based on test type
        const int order = h_convergence ? 1 : (it + 1);          // *** CHANGED
        const int ref_levels_it = h_convergence ? it : ref_levels; // *** CHANGED
