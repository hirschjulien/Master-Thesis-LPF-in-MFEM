...
// Weak Scaling experiment. Uses meshes "wave-tank-big{ ,2,4,8}.msh".
// Measures max wall clock runtime over all ranks for keeping workload constant over the ranks
...
static const char *WeakMeshForRanks(const int np)
{
    switch (np)
    {
        case 1: return "../Meshes/wave-tank-big.mesh";
        case 2: return "../Meshes/wave-tank-big2.mesh";
        case 4: return "../Meshes/wave-tank-big4.mesh";
        case 8: return "../Meshes/wave-tank-big8.mesh";
        default:
            // Fallback: use the 1x mesh if you run other ranks
            return "../Meshes/wave-tank-big.mesh";
    }
}
...
int main(int argc, char *argv[])
{
    ...
    // mode = 1 -> weak scaling   (grow problem with ranks using different meshes)
    // ============================
    const int mode = 1;
    ...
    const int par_ref_levels_weak = 0; // keep 0 for weak scaling
    ...
    if (myid == 0)
    {
        fout.open("data/weak-scaling.txt", ios::app);
    ...
    {
        // Strong scaling
        mesh_file = "../Meshes/wave-tank-big.mesh";
        par_ref_levels = par_ref_levels_fixed;
    }
    else
    {
        // Weak scaling: use different mesh files for 1/2/4/8 ranks
        mesh_file = WeakMeshForRanks(num_procs);
        par_ref_levels = par_ref_levels_weak; // keep 0
    }
