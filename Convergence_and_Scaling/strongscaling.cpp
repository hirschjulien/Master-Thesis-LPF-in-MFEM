...
int main(int argc, char *argv[])
{
    ...
    // --- CHANGES START ---
    const int par_ref_levels_fixed = 2;   // keep par_ref_level fixed across runs (set as you want)
    // --- CHANGES END ---
    ...
    {
        // --- CHANGES START ---
        fout.open("data/strong_scaling_rk4.txt", ios::app);
        fout << "# order  par_ref_level  dofs  runtime[s]\n";
        cout << "Strong scaling test (fixed problem)\n";
    ...
    double t = 0.0;
    const double t_final_placeholder = 1.0;
    ...
    // --- CHANGES START ---
    // Run for order = 3 and 4, with par_ref_level fixed
    for (int order : {3, 4})
    {
        const int par_ref_levels = par_ref_levels_fixed;
        // --- CHANGES END ---
        ...
        // Measure time stepping only (fixed workload). Use max over ranks.
        ...
        if (myid == 0)
        {
            ...
            fout << order << "  " << par_ref_levels << "  " << global_ndofs << "  " << max_time << "\n";
            // --- CHANGES END ---
        }
