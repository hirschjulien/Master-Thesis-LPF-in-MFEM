...
int main (int argc, char *argv[])
{
    ...
    if (myid == 0)
    {
        std::ifstream fin("data/laplace-hconv-phi.txt");
        const bool empty = (fin.peek() == std::ifstream::traits_type::eof());
        fin.close();

        fout.open("data/laplace-parallel-hconv.txt", std::ios::out | std::ios::app);
        if (empty) { fout << "order dofs elements phi-inf-error\n"; }
    }



for(int ref_levels=0; ref_levels<=4; ref_levels++){
    int order = 4;
    //int ref_levels = 0;
    ...
    FiniteElementCollection *fec2 = new H1_FECollection(order, dim); // H1 Lagrange elements, pointer "fec" resides on heap, has to be deallocated again in the end
    FiniteElementSpace fespace2(&mesh_serial, fec2);  // FE space is now parallel and lives on ParMesh
    ...
    cg.SetMaxIter(2000);          // max iterations
    cg.Mult(B, X);               // Solve
    ...
    // cout << "Inf Phi Error: " << l2 << endl;
        ...
        if (myid == 0)
        {
            ...
            fout << order << " " << ndofs << " " << ne << " " << global_err << "\n";
        }
