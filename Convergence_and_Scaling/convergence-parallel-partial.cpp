...
int main(int argc, char *argv[])
{
    ...
    ofstream fout;
    if (myid == 0){
        fout.open("data/pf-parallel-pconv-eta.txt");
        fout << "# order dofs phi_inf_error ";
    }
    ...
    for (int order = 1; order <= 8; order++)
    {
        ...
        if(order == 1 && myid==0){
        fout << "k=" << k << ", kh=" << kh << ", omega=" << omega << ", T=" << T << ", cwave=" << cwave << " ";
        }
    ...
    FunctionCoefficient w_exact([&](const Vector& X){
        ...
        return -H * cwave * 0.5 * k * (sinh(k*(z_rel+h)) / sinh(k*h)) * sin(omega*t - k*(kx_dir * X(0) + ky_dir * X(1)));
    });
...
//         double local_err_w = w_num.ComputeMaxError(w_exact);
//         double global_err_w = 0.0;
//         MPI_Allreduce(&local_err_w, &global_err_w, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);

//         int local_ndofs_w = fespace.GetTrueVSize();
//         int global_ndofs_w = 0;
//         MPI_Allreduce(&local_ndofs_w, &global_ndofs_w, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
...
//             fout << order << " " << global_ndofs_w << " " << global_err_w << "\n";
//         }
